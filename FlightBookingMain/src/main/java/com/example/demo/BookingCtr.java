...
public class BookingCtr {
	...
	@Autowired
	private MyProxyForConfig myProxyConf;
	...
	ResponseEntity<Object> bookTickets(@RequestBody Booking b, @PathVariable("fid") Long flightId) {
		...
		if(res.isPresent()) {
			...
			if(bookedSeats+b.getSeatsBooked() > f1.getTotalSeats()) {
				return new ResponseEntity<Object>(new MyJSONWrapper("Error", "Available Seats = " +  (f1.getTotalSeats()-bookedSeats)), HttpStatus.BAD_REQUEST);										
			}			
			
			
			Map maxConf = (Map)myProxyConf.getMAxFromConf().getBody();
			Map flexiConf = (Map)myProxyConf.getFlexiFromConf().getBody();
			double baserate = f1.getBasePrice();
			double flexrate = 0;
			TreeSet<Double> ts = new TreeSet();
			for(Object key : flexiConf.keySet()) {
				ts.add(new Double(key.toString()));
			}
			for(Double d1 : ts) {
				if(bookedSeats>d1) {
					flexrate = new Double(flexiConf.get(d1.intValue()+"").toString());
				}
			}
			Double maxAllowed = new Double(maxConf.get("message").toString());
			if(b.getSeatsBooked()>maxAllowed) {
				return new ResponseEntity<Object>(new MyJSONWrapper("Error", "Max Booking Allowed For = " +  maxAllowed), HttpStatus.BAD_REQUEST);														
			}
			...
			b.setStatus("Booked");
			b.setTotalPrice((f1.getBasePrice() + flexrate) *b.getSeatsBooked());
			Authentication auth = SecurityContextHolder.getContext().getAuthentication();
